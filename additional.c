...
t_point transform_point(t_fdf *fdf, t_map *map, int row, int column)
{
    ...
    map_middle_x = (double)(map->width / 2);    
    map_middle_h = (double)(map->height / 2);
...
void put_pixels(t_fdf *fdf)
{
    ...
    while (--row >= 0)
    {
        ...
        while (--column >= 0)
        {
            ...
            fdf->map.points[index_matr(row, column, map_w)] = point;
            img_pixel_put(&fdf->image, point.x, point.y, 0xffffff);
        }
    }
}
...
void render_image(t_fdf *fdf)
{
    ...
    while (--row > -1)
	{
		...
		while (--column > -1)
		{
			point = fdf->map.points[index_matr(row, column, map_w)];
			if (row + 1 < fdf->map.height)
				draw_line(fdf, point, fdf->map.points[index_matr(row + 1, column, map_w)]);
			if (column + 1 < map_w)
				draw_line(fdf, point, fdf->map.points[index_matr(row, column + 1, map_w)]);
		}
	}
